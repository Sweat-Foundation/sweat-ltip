...
test('T07 ? Authorize 100%', async t => {
  ...
  // When: Executor calls authorize(grant_id = G1, percent = 100)
  await executor.call(contract, 'authorize', {
    account_ids: [alice.accountId],
    percentage: 10_000 // 100%
  });

  // Then: Authorized amount = 400, order_amount = 0, claimed_amount = 600, spare unchanged, transfer 400
  ...
  t.is(aliceBalanceAfter - aliceBalanceBefore, orderAmount);
});
