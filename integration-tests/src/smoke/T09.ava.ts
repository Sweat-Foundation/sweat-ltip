...
test('T09 ? Buy partial (e.g. 50%)', async t => {
  ...
  // When: Executor calls buy(grant_id = G1, percent = 50)
  await executor.call(contract, 'buy', {
    account_ids: [alice.accountId],
    percentage: 50_00 // 50%
  });

  // Then: Bought amount = 200, order_amount = 0, claimed_amount = 400, spare increases by 200
  ...
  const boughtAmount = orderAmount / 2n;
  const spareAfter = BigInt(await contract.view('get_spare_balance'));
  t.is(spareAfter, spareBefore + boughtAmount);
