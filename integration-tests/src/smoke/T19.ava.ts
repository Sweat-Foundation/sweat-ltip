...
test('T19 ? Terminate during vesting, with orders', async t => {
  const { contract, ft, issuer, executor, alice } = t.context.accounts;

  // Given: T_CLIFF_END < T_TERM < T_VEST_END, claimed_amount = 200, order_amount = 400, vested_at_T_TERM = 500
  await issuer.call(
    ft, 'ft_transfer_call',
  ...
  const T_TERM = T_ISSUE + 600; // Termination 600 seconds after issue (vested ~500)
  ...
  // Then: G1.total_amount = 500, order_amount reduced to 300 (claimed + order ? total)
  const account: Account = await contract.view('get_account', { account_id: alice.accountId });
  const G1 = account.grants.at(0)!;
  const vestedAtTerm = BigInt(G1.vested_amount);
