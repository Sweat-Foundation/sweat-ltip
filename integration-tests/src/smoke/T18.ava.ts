...
test('T18 ? Terminate during vesting, no orders', async t => {
  const { contract, ft, issuer, executor, alice } = t.context.accounts;

  // Given: T_CLIFF_END < T_TERM < T_VEST_END, claimed_amount = 200, order_amount = 0, vested_at_T_TERM = 550
  await issuer.call(
    ft, 'ft_transfer_call',
  ...
  const T_ISSUE = now() - 700; // Issue 700 seconds ago
  ...
  const T_TERM = T_ISSUE + 650; // Termination 650 seconds after issue (vested ~550)
  ...
  // Then: G1.total_amount = 550, spare increases by 450
