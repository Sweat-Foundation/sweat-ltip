...
test('Check authorize', async t => {
  ...
  {
    console.log(`  ➤ Call contract.authorize by unauthorized account`);
  ...
  {
    console.log(`  ➤ Call contract.authorize by authorized account`);
    const result = await executor.call(contract, 'authorize', { account_ids: [alice.accountId], percentage: 10_000 });
    console.log('    ↩ Result:', result);
